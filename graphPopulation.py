...
from neomodel import StructuredNode, StringProperty, RelationshipTo, RelationshipFrom, config, StructuredRel, db
import pandas as pd
config.DATABASE_URL = 'bolt://neo4j:123456a@@localhost:7687'
...
class GraphPopulation:
    ...
    def get_all_triple(self):
        results, columns = db.cypher_query("MATCH (s)-[r]->(o) RETURN s, r, o")
        subjects = []
        objs = []
        rels = []
        for result in results:
            s = Subject.inflate(result[0])
            r = RelationshipModel.inflate(result[1])
            o =Object.inflate(result[2])
            subjects.append(s.subject_name)
            rels.append(r.relationType)
            objs.append(o.object_name)
        df = pd.DataFrame({"subject": subjects, "relation": rels, "obj": objs})
        return df
